...
import jwt
import datetime
from MentalHealthApp.models import User, Test, Question,QuestionAnswer,TestSubmission
...
# class DeleteTestViewTestCase(TestCase):
#     def setUp(self):
#         self.client = APIClient()
#         self.user = User.objects.create(email='test@example.com', password='password')
#         self.test_obj = Test.objects.create(name='Test na zmazanie', created_by=self.user)
#         self.delete_url = f'/api/tests/{self.test_obj.id}/'  # prispôsob podľa tvojej route

#     def test_delete_existing_test(self):
...
#         non_existing_url = f'/api/tests/9999/'
